...
/**
 * This scheduler sends a new job into an available instance by selecting among
 * them randomly.
 * 
 ...
 *
 */
public class RandomScheduler implements ITestJobScheduler {

    private Logger logger = Logger.getLogger(RandomScheduler.class);
    ...
    // Shall we use a default value here to ensure replicability ?
    private static final long RANDOM_SEED = 3237799987L;
	...
	for (Job cloudJob : jobs) {
	    ...
	    // Randomly select an instance and the already allocated jobs
	    int index = random.nextInt(reservedInstances.length);
	    List<Job> _jobs = jobsDistribution.get(reservedInstances[index]);
	    // This job start when the last one in this instance finishes, if
	    // any
